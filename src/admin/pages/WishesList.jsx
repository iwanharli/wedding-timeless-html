...
function DeleteWishModal({ entry, onConfirm, onCancel, deleting }) {
  return (
    <div className="wl-confirm-backdrop" onClick={onCancel}>
      <div className="wl-confirm-modal" onClick={e => e.stopPropagation()}>
        <div className="wl-confirm-icon"><i className="fas fa-trash-alt" /></div>
        <h3 className="wl-confirm-title">Hapus Ucapan Ini?</h3>
        <p className="wl-confirm-sub">Tindakan ini tidak dapat dibatalkan.</p>

        <div className="wl-confirm-preview">
          <div className="wl-avatar wl-avatar--sm" style={{ background: avatarColor(entry.name) }}>
            {initials(entry.name)}
          </div>
          <div className="wl-confirm-preview-body">
            <span className="wl-confirm-preview-name">{entry.name}</span>
            {entry.wish?.trim() && <p className="wl-confirm-preview-wish">&ldquo;{entry.wish}&rdquo;</p>}
          </div>
        </div>
...
export default function WishesList({ onMenuOpen }) {
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    try {
      const res = await authFetch(`/api/rsvp/${deleteTarget.id}`, { method: 'DELETE' })
      ...
      <div className="db-stat-grid db-stat-grid--5">
        <div className="db-stat-card">
          ...
          <div className="db-stat-body">
            <div className="db-stat-value">{attending}</div>
            <div className="db-stat-label">Hadir</div>
          </div>
        </div>
        <div className="db-stat-card">
          ...
          <div className="db-stat-body">
            <div className="db-stat-value">{maybe}</div>
            <div className="db-stat-label">Mungkin</div>
          </div>
        </div>
        <div className="db-stat-card">
          ...
          <div className="db-stat-body">
            <div className="db-stat-value">{notAttending}</div>
            <div className="db-stat-label">Tidak Hadir</div>
          </div>
        </div>
        <div className="db-stat-card">
          ...
          <div className="db-stat-body">
            <div className="db-stat-value">{visibleCount}</div>
            <div className="db-stat-label">Tampil Publik</div>
            <div className="db-stat-sub">ucapan terlihat</div>
          </div>
        </div>
        <div className="db-stat-card">
          ...
          <div className="db-stat-body">
            <div className="db-stat-value">{hiddenCount}</div>
            <div className="db-stat-label">Tersembunyi</div>
            <div className="db-stat-sub">disembunyikan admin</div>
          </div>
        </div>
      </div>
          ...
          <table className="gl-table">
            <thead>
              <tr>
                ...
                <th className="gl-th gl-th--action" title="Hapus"><i className="fas fa-trash-alt" /></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((e, i) => (
                    ...
                    <div key={e.id} className={`wl-card${e.visible === false ? ' wl-card--hidden' : ''}`}>
                      <div className="wl-card-top">
                        ...
                        <div className="wl-card-actions">
                          ...
                          <button
                            type="button"
                            className="gl-icon-btn gl-icon-btn--danger wl-del-btn"
                            onClick={() => setDeleteTarget(e)}
                            title="Hapus ucapan"
                          >
                            <i className="fas fa-trash-alt" />
                      ...
                      <p className={`wl-card-wish${expanded.has(e.id) ? ' wl-card-wish--expanded' : ''}`}>{e.wish}</p>
                      {e.wish.length > 140 && (
                        <button type="button" className="wl-wish-toggle" onClick={() => toggleExpand(e.id)}>
                          {expanded.has(e.id) ? 'Sembunyikan' : 'Baca selengkapnya'}
                        </button>
                      )}
                      {e.guests > 1 && (
